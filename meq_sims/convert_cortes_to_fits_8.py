...
for i in range(num_phi):
    ...
    for j in range(num_theta):
      ...
      total_ampl = math.sqrt(theta_ampl*theta_ampl+phi_ampl*phi_ampl)
...
if poln:
  ...
  total_data_ampl = numpy.array(cx_ampl,'f')
else:
...
result_total = griddata(l,m,total_data_ampl,xo,yo)
...
if poln:
  # CS = plt.contourf(xo,yo,result_total,15,cmap=plt.cm.jet)
  CS = plt.contourf(xo,yo,result_total,15,locator=ticker.LogLocator(),cmap=plt.cm.jet)
  ...
  # CS = plt.contourf(xo,yo,result_E,15,cmap=plt.cm.jet)
else:
  # CS = plt.contourf(xo,yo,result_total,15,cmap=plt.cm.jet)
  CS = plt.contourf(xo,yo,result_total,15,locator=ticker.LogLocator(),cmap=plt.cm.jet)
...
title = filename + ' total mag. of p.b.r.,log color scale'
# title = filename + ' cross-pol, real part of p.b. response'
plt.title(title,fontsize=8.2)
ax=plt.subplot(122)
if poln:
  # CS = plt.contourf(xo,yo,result_total*result_total,15,cmap=plt.cm.jet)	
  CS = plt.contourf(xo,yo,result_total*result_total,15,locator=ticker.LogLocator(),cmap=plt.cm.jet)
  #CS = plt.contourf(xo,yo,result_phi**2,15,cmap=plt.cm.jet)
  #CS = plt.contourf(xo,yo,result_real_co,15,cmap=plt.cm.jet)
  #CS = plt.contourf(xo,yo,result_imag_co,15,cmap=plt.cm.jet)
else:
...
plt.xlabel('M',fontsize=8.5)
plt.ylabel('L',fontsize=8.5)
title = filename + ' total mag. power pattern (|E|^2),log color scale'
#title = filename + ' co-pol, real part of p.b. response'
plt.title(title,fontsize=8.0)
save_file = str(filename) + '_ant_pats_total_mag_log_color.pdf'
