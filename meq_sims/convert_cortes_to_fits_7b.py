...
for i in range(num_phi):
    ...
    for j in range(num_theta):
	  ...
	  cx_total_ampl.append(total_ampl)
	  theta1.append(theta)
          phi1.append(phi)
...
if poln:
  total_ampl_data=numpy.array(cx_total_ampl,'f')	
...
theta11=numpy.array(theta1,'f')
phi11=numpy.array(phi1,'f')
l_min = -0.15
l_max = 0.15
m_min = -0.15
m_max = 0.15
...
if poln:
  ...
  # ax.scatter(l,m,ampl_total)
  # ax.scatter(l,m,total_ampl_data)
  # ax.scatter(theta11,phi11,ampl_theta_db_data)
  # ax.scatter(theta11,phi11,total_ampl_data)
  # ax.scatter(theta11,phi11,ampl_theta)
  # ax.scatter(theta11,phi11,ampl_theta*ampl_theta)
  # ax.scatter(theta11,phi11,real_data_cx)
  ax.scatter(theta11,phi11,imag_data_cx)
...
ax.set_xlabel('L, total amplitude')
ax.set_ylabel('M')
# ax.set_zlabel(r'$E_{\phi}$ (db)')
# ax.set_zlabel(r'$E_{\phi}^{2}$')
ax.set_zlabel(r'$|E_{total}|$')
#plt.xlabel('M',fontsize=8.5)
#plt.ylabel('L',fontsize=8.5)
# title = filename + ' cross-pol'
title = filename + ' cross-pol, total amplitude'
# ax.text(x,y,z,s,title)
# ax.text3D(title)
ax.set_title(title)
plt.title(title)
save_file = str(filename) + '_ant_pats_pb_E_total_3d_scatter1.pdf'
