...
if poln:
  ...
  # X = np.arange(l_min,l_max)
  # Y = np.arange(m_min,m_max)
  X,Y=np.meshgrid(l,m)
  ax.plot_surface(X,Y,ampl_theta_db_data,rstride=1, cstride=1, cmap=cm.jet)
...
ax.set_title(title)
plt.title(title)
save_file = str(filename) + '_ant_pats_pb_imag_3d_surface1.pdf'
